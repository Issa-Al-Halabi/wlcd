    ...
    <form action="{{ route('course.update', $cor->id) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="col-md-3">
                ...
                <select name="category_id" id="category_id" class="form-control js-example-basic-single" required>
                    ...
                    @foreach ($category as $caat)
                        <option {{ $cor->category_id == $caat->id ? 'selected' : '' }} value="{{ $caat->id }}">
                            {{ $caat->title }}</option>
                    @endforeach
                </select>
            </div>
                ...
                <select name="user_id" class="form-control js-example-basic-single col-md-7 col-xs-12">
                    @if (Auth::user()->role == 'admin')
                        @foreach ($users as $user)
                            <option {{ $cor->user_id == $user->id ? 'selected' : '' }} value="{{ $user->id }}">
                                {{ $user->fname }}</option>
                        @endforeach
                    @else
        ...
        <div class="row">
            <div class="col-md-4">
                ...
                <select name="language_id" class="form-control js-example-basic-single col-md-7 col-xs-12">
                    ...
                    @foreach ($languages as $cat)
                        <option {{ $cor->language_id == $cat->id ? 'selected' : '' }} value="{{ $cat->id }}">
                            {{ $cat->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="col-md-4">
                ...
                <select name="course_type" class="form-control">
                    <option value="1" {{ $cor->course_type == '1' ? 'selected' : '' }}>
                        Online</option>
                    <option value="2" {{ $cor->course_type == '2' ? 'selected' : '' }}>
                        Record</option>
                </select>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <input type="text" class="form-control" name="title" id="exampleInputTitle"
                    value="{{ $cor->getTranslation('title', 'en') }}">
            </div>
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <textarea name="short_detail" rows="3" class="form-control">{!! $cor->getTranslation('short_detail', 'en') !!}</textarea>
            </div>
            <div class="col-md-6">
                <label for="exampleInputDetails">{{ __('Requirements') }}:<sup class="redstar">*</sup></label>
                <textarea name="requirement" rows="3" class="form-control" required>{!! $cor->getTranslation('requirement', 'en') !!}</textarea>
            </div>
        </div>
        <br>
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <select class="form-control js-example-basic-single" name="level_tags">
                        ...
                        <option {{ $cor->level_tags == 'bestseller' ? 'selected' : '' }} value="bestseller">
                            {{ __('Bestseller') }}</option> --}}

                        <option {{ $cor->getTranslation('level_tags', 'en') == 'beginner' ? 'selected' : '' }}
                            value="beginner">
                            {{ __('Beginner') }}</option>

                        <option {{ $cor->getTranslation('level_tags', 'en') == 'intermediate' ? 'selected' : '' }}
                            value="intermediate">
                            {{ __('Intermediate') }}</option>

                        <option {{ $cor->getTranslation('level_tags', 'en') == 'expert' ? 'selected' : '' }}
                            value="expert">
                            {{ __('Expert') }}</option>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input value="{{ $cor['start_date'] }}" type="text" id="start-date1"
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input value="{{ $cor['end_date'] }}" type="text" id="end-date1"
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <textarea id="detail" name="detail" rows="3" class="form-control">{!! $cor->getTranslation('detail', 'en') !!}</textarea>
            </div>
        </div>
        <br>
        ...
        <div style="{{ $cor->day == 1 ? '' : 'display:none' }}" id="jeet">
            ...
            <div class="col-md-3">
                ...
                <div style="{{ $cor->type == 1 ? '' : 'display:none' }}" id="doabox">
                    ...
                    <input step="0.01" type="text" inputmode="numeric" pattern="[-+]?[0-9]*[.,]?[0-9]+"
                        class="form-control" name="price" id="priceMain"
                        placeholder="{{ __('Enter') }} {{ __('Price') }}" value="{{ $cor->price }}">

                    <br>
                    <label for="exampleInputSlug">{{ __('DiscountPrice') }}: <sup class="redstar">*</sup></label>
                    <input step="0.01" type="text" inputmode="numeric" pattern="[-+]?[0-9]*[.,]?[0-9]+"
                        class="form-control" name="discount_price" id="exampleInputPassword1"
            ...
            <div class="col-md-4">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <div class="custom-file">
                            <input type="file" class="custom-file-input" id="inputGroupFile01" name="video"
                                value="{{ $cor->video }}" aria-describedby="inputGroupFileAddon01">
                ...
                <div @if ($cor->preview_type == 'video') class="display-none" @endif id="document2">
                    ...
                    <input type="url" class="form-control" placeholder="{{ __('Enter') }} URL"
            ...
            <div class="col-md-4">
                ...
                <input min="1" class="form-control" name="duration" type="number" id="duration"
                    value="{{ $cor->duration }}" placeholder="{{ __('Enter') }} {{ __('Duration') }}">
            </div>
        </div>
        <br>
